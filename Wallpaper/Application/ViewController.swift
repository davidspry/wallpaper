...
class ViewController: NSViewController {
    ...
    @IBOutlet var metalKitView: MetalKitView!
    @IBOutlet var loadingAnimation: LoadingAnimation!
    ...
    func loadTextures(fromUrls fileUrls: [URL]) {
        guard let renderer = renderer,
              ...
              let metalKitDevice = metalKitView.device
        else {
            return
        }
    ...
    override func rightMouseDown(with event: NSEvent) {
        guard let delegate = NSApplication.shared.delegate as? AppDelegate,
              let sourceTextures = renderer?.sourceTextures
        else {
            return
        }
